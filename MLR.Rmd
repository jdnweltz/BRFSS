...
summary(brfssMod)
```
While all variables are significant, our overall model is not that precise, as we have a $R^2$ Value of 0.1099, and an Adjusted-$R^2$ Value of 0.1098. Both our values of $R^2$ and Adjusted-$R^2$ tell us that approximately 11% of the variability in days of poor mental health is explained by the model, which is not a very good fit. There is no guarantee that our model wil accurately describe the population, and this is due to the size of our population. There is so much natural variability that occurs in the American population, and our data is from such a large sample size, that there will always be a ton of unexplained variability in our model, regardless of how precise it is.

### Residuals

From the residual plot that was given earlier, it was evident that our data did not have normal variance centered around 0. This could very well be due to influence points that pull the line askew.
...
It appears that points with values for number of drinks of 45, 70, 73, 75, and 76 have extremely low residuals, and points with values of hours of sleep greater than 11 have extremely high residuals.
...
```
While our residuals are still not normally distributed around 0, we do have a fit more condensed in the Y-direction, as our residuals range from -4 to 4 as opposed to -5 to 5 as was the case before. While it would be desirable to have residuals perfectly centered around 0, it seems that this is pretty unrealistic due to the sheer number of datapoints we have. Our sample size has so many observations that there is so much natural variability that cannot possibly be taken into account by a single line.
...
```
```{r}
#add1(lm(new_menthlth~new_genhlth + new_sleptim1 + new_sex + new_avedrnk2 + new_sex*new_avedrnk2 + new_veteran3 + new_sex*new_veteran3 + new_veteran3*new_avedrnk2, data=use_entries), new_menthlth ~ new_genhlth + new_sex + new_veteran3 + new_sleptim1 + new_avedrnk2 + new_sex*new_avedrnk2 + new_sex*new_veteran3 + new_veteran3*new_avedrnk2, test = "F")
```
...
This will be done once we are sure that we have the correct model, and that the forward selection was done correctly.
...
```
We are 95% confident that the true mean value for days of poor mental health for a male veteran who had 10 drinks in the past 30 days, gets an average of 5 hours of sleep every 24 hours, and reports fair general health is between 8.318683 and 8.842484 days in the past 30 days.

```{r}
predict.lm(brfssModReduced, newdata, interval = "predict")
```
We are 95% confident that a male veteran who had 10 drinks in the past 30 days, gets an average of 5 hours of sleep every 24 hours, and reports fair general health will have had between -4.449452 (0) and 21.61062 days of poor mental health in the past 30 days.
...
I have to do with this updated model, but want to check if I'm doing this correctly first.

have to create new anova table for each calculation, adding variable of interest last for each one.
```{r}
anova(brfssMod)
```
Total SSE: 1272580

new_genhlth: 986124/286456 = 3.442497
How does this make sense??

new_sex: 88080/1184500 = 0.07436049
7.44% of the variation in days of poor mental health is reduced due to sex being added to the model.
Is this the correct way to interpret this??

new_veteran3: 8228/1264352 = 0.006507681

new_sleptim1: 116517/1156063 = 0.1007878

new_avedrnk2: 62786/1209794 = 0.05189809

new_sex:new_avedrnk2: 8520/1264060 = 0.006740186

new_sex:new_veteran3: 1917/1270663 = 0.001508661

new_veteran3:new_avedrnk2: 408/1272172 = 0.0003207114

### Summary

Questions:

R^2 vs adj - interpret R^2, not adjusted. adjusted just used for selection
coeffient of partial determination - see definition below
forward selections results - totally fine
influence points - no need to do anything more here. Hardin said that our model isn't ever going to be perfect, so what we have is already good, we just need to explain it, which I feel we do.
prediction intervals - doesn't make too much sense to do adjustments, as we're only creating one interval now, as opposed to how we made multiple intervals before

definition

Id you have 2,3,4,5, adding 1 how much does it reduce SSE
I would also look at her hw solutions to answer this question




Things to do before next meeting with Hardin:
change interpretation of R^2
fix coefficient of partial determination
add interpretation/analysis of forward selection and model interpretation as a whole
