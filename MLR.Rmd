...
```
Through forward selection, we end up with the same variables in the model as we had before. 

### Model Interpretation

Started with 5 variables (plues 3 interaction terms)
Best subsets returned a model with all 8 terms
The tried forward selection to see if there was any difference - there was not
Concluded that a model with all terms is the best model to use, as all 8 terms are significant, but this still doesn't produce the most accurate results as our data is so dense and variate.

We started with our five variables, plus three interaction terms, and performed best subsets on these variables in order to get the best model. This selection process resulted in a model where all eight terms were significant. We then attempted to build the model using forward selection, which resulted in the same model with eight terms that was obtained through best subsets. Since both selection processes resulted in the same model, we concluded that this "full" model was the best model to use. But even though all eight terms are significant, this model still does not produce the most accurate predictions as seen by our $R^2$ value.
